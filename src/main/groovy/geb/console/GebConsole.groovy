...
/**
 * Blatant copy/paste of groovy.ui.Console! Only differences:
 *  - labels say 'GebConsole' instead of 'GroovyConsole'
 *  - Uses ContextualGroovyShell instead of GroovyShell
 *
 * =========================== groovy.ui.Console JavaDoc =======================================
 * Groovy Swing console.
 *
 * Allows user to interactively enter and execute Groovy.
 *
 * @version $Id: Console.groovy 20314 2010-06-21 15:01:03Z roshandawrani $
 * @author Danno Ferrin
 ...
 * @author Paul King
 */
class GebConsole implements CaretListener, HyperlinkListener, ComponentListener, FocusListener {

    static final String DEFAULT_SCRIPT_NAME_START = "ConsoleScript"

    static private prefs = Preferences.userNodeForPackage(GebConsole)
    ...
    def scriptFile
    File currentFileChooserDir = new File(Preferences.userNodeForPackage(GebConsole).get('currentFileChooserDir', '.'))
    File currentClasspathJarDir = new File(Preferences.userNodeForPackage(GebConsole).get('currentClasspathJarDir', '.'))
    File currentClasspathDir = new File(Preferences.userNodeForPackage(GebConsole).get('currentClasspathDir', '.'))
    ...
    static void main(args) {
        if (args.length == 1 && args[0] == '--help') {
            println '''usage: GebConsole [options] [filename]
options:
  ...
  -cp,-classpath,--classpath <path>    Specify classpath'''
        ...
        def console = new GebConsole(GebConsole.class.classLoader?.getRootLoader())
        console.run()
    ...
    GebConsole(Binding binding) {
        this(null, binding)
    }

    GebConsole(ClassLoader parent) {
        this(parent, new Binding())
    }

    GebConsole(ClassLoader parent, Binding binding) {
    ...
    void newScript(ClassLoader parent, Binding binding) {
        shell = new ContextualGroovyShell(parent, binding)
    }
    ...
    boolean askToSaveFile() {
        ...
        switch (JOptionPane.showConfirmDialog(frame,
            ...
            "GebConsole", JOptionPane.YES_NO_CANCEL_OPTION))
        {
    ...
    def askToInterruptScript() {
        ...
        def rc = JOptionPane.showConfirmDialog(frame, "Script executing. Press 'OK' to attempt to interrupt it before exiting.",
            "GebConsole", JOptionPane.OK_CANCEL_OPTION)
    ...
    void fileNewWindow(EventObject evt = null) {
        GebConsole consoleController = new GebConsole(
            new Binding(
    ...
    void addClasspathJar(EventObject evt = null) {
        ...
        if (fc.showDialog(frame, "Add") == JFileChooser.APPROVE_OPTION) {
            ...
            Preferences.userNodeForPackage(GebConsole).put('currentClasspathJarDir', currentClasspathJarDir.path)
    ...
    void addClasspathDir(EventObject evt = null) {
        ...
        if (fc.showDialog(frame, "Add") == JFileChooser.APPROVE_OPTION) {
            ...
            Preferences.userNodeForPackage(GebConsole).put('currentClasspathDir', currentClasspathDir.path)
            shell.getClassLoader().addURL(fc.selectedFile.toURL())
        }
    }
    ...
    def selectFilename(name = "Open") {
        ...
        if (fc.showDialog(frame, name) == JFileChooser.APPROVE_OPTION) {
            ...
            Preferences.userNodeForPackage(GebConsole).put('currentFileChooserDir', currentFileChooserDir.path)
    ...
    void showAbout(EventObject evt = null) {
        ...
        pane.setMessage('Welcome to the Geb Console for evaluating Geb scripts\nVersion ' + version)
        def dialog = pane.createDialog(frame, 'About GebConsole')
        dialog.show()
    }
    ...
    void updateTitle() {
        if (frame.properties.containsKey('title')) {
            if (scriptFile != null) {
                frame.title = scriptFile.name + (dirty?" * ":"") + " - GebConsole"
            } else {
                frame.title = "GebConsole"
            }
        }
    }
